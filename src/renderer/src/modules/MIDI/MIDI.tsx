  ...
  useEffect(() => {
    if (midi) {
      ...
      function onEnabled() {
        ...
        // Outputs
        WebMidi.outputs.forEach((output) => log.success3(output.manufacturer, output.name))
      }
    } else {
      WebMidi.disable()
    }
    return () => {
      if (midi) {
        WebMidi.disable().then(() => log.success('WebMidi with sysex disabled!'))
      }
    }
  }, [midi])
...
export const Settings = () => {
  return (
    <>
      <ToggleSettings name="midi" iconOn={<Piano />} iconOff={<PianoOff color="disabled" />} />
    </>
  )
}
