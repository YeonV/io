...
const Settings = () => {
  ...
  return (
    <Box sx={{ mb: 5, mt: 2.5, maxWidth: 300, width: '100%' }}>
      <Box sx={{ mb: 5, mt: 2.5, mr: 'auto', ml: 'auto', maxWidth: 100, textAlign: 'center' }}>
        ...
        <FormGroup>
          <FormControlLabel labelPlacement="start" control={<>{inputs.mqtt && <Popup />}<Switch value={inputs.mqtt && outputs.mqtt} onChange={() => { toggleInput('mqtt'); toggleOutput('mqtt') }} /></>} label="MQTT" />
